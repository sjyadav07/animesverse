...
import TrendingAnimeHomeCard from "../../card/Home/TrendingAnimeHomeCard";

const TrendingHomeAnime = ({ trendingAnimes }) => {
  ...
  return (
    <section className="my-6 px-4">
      <div className="flex justify-between my-4">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm md:text-base text-gray-400 font-medium tracking-wide">
            The anime everyone’s watching right now.
          </p>
        </div>

        <NavLink
          to="/anime-list/trending"
      ...
      >
        {trendingAnimes.map((item) => (
          <SwiperSlide key={item.mal_id}>
            <TrendingAnimeHomeCard currEle={item} />
          </SwiperSlide>
        ))}
...
export default TrendingHomeAnime;
