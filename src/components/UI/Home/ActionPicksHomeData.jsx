...
import ActionPicksHomeCard from "../../card/Home/ActionPicksHomeCard";

const ActionPicksHomeData = ({ actionPicks }) => {
  ...
  return (
    <section className="my-6 px-4">
      <div className="flex justify-between my-4">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm md:text-base text-gray-400 font-medium tracking-wide">
            High-stakes battles start here.
          </p>
        </div>

        <NavLink
          to="/anime-list/action-picks"
      ...
      >
        {actionPicks.map((item, idx) => (
          <SwiperSlide key={`${item.mal_id}-${idx}`}>
            <ActionPicksHomeCard currEle={item} />
          </SwiperSlide>
        ))}
...
export default ActionPicksHomeData;
