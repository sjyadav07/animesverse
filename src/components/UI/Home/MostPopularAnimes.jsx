...
import MostPopularHomePageCards from "../../card/Home/MostPopularHomePageCards";

const MostPopularAnimes = ({ mostPopularAnimeHomeData }) => {
  const [isBeginning, setIsBeginning] = useState(true);
  const [isEnd, setIsEnd] = useState(false);
  return (
    <section className="my-6 px-4">
      {/* <div className="flex justify-between my-5">
        <h2 className="  text-xl sm:text-3xl font-bold">Most Popular Animes</h2>
        <NavLink
          to="/anime-list/most-popular"
      ...
      <div className="flex justify-between my-4">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm md:text-base text-gray-400 font-medium tracking-wide">
            The most talked-about anime worldwide.
          </p>
        </div>
      ...
      >
        {mostPopularAnimeHomeData.map((item) => (
          <SwiperSlide key={item.mal_id}>
            <MostPopularHomePageCards currEle={item} />
          </SwiperSlide>
        ))}
