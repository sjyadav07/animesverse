...
import SeasonalHomeCardList from "../../../components/card/Home/SeasonalHomeCardList";

const SeasonalSection = ({ seasonalData }) => {
      ...
      <div className="flex justify-between my-4">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm md:text-base text-gray-400 font-medium tracking-wide">
            Brand-new stories just dropped.
          </p>
        </div>
      ...
      >
        {seasonalData.map((item) => (
          <SwiperSlide key={item.mal_id}>
            <SeasonalHomeCardList currEle={item} />
          </SwiperSlide>
        ))}
